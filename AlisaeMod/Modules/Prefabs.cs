...
namespace AlisaieMod.Modules
{
    ...
    internal static class Prefabs
    {
        ...
        public static void SetupCustomRendererInfos(CharacterModel characterModel, CustomRendererInfo[] customInfos)
        {
            ...
            {
                var childName = customInfos[i].childName;
                var child = childLocator.FindChild(childName);
                if (!child)
                {
                    Log.Error($"[RendererDebug] Could not find child '{childName}' in ChildLocator.");
                }
                else
                {
                    ...
                    if (!rend)
                    {
                        Log.Error($"[RendererDebug] Child '{childName}' found, but has NO Renderer component.");
                    }
                    else
                    {
                        Log.Message($"[RendererDebug] Child '{childName}' found, Renderer type: {rend.GetType().Name}, adding to baseRendererInfos.");
            ...
            Log.Message($"[RendererDebug] Final baseRendererInfos count: {characterModel.baseRendererInfos.Length}");
            for (int i = 0; i < characterModel.baseRendererInfos.Length; i++)
            {
                Log.Message($"[RendererDebug] baseRendererInfos[{i}]: {characterModel.baseRendererInfos[i].renderer?.gameObject.name}");
            }
        }
